...
class Calibrator {
 public:
  ...
  Mat get_camera_matrix(){return this->camera_matrix;}
  Mat get_dist_coeffs(){return this->dist_coeffs;}
  bool read_intrinsics_(const string filename);
  bool write_intrinsics_(const string filename);
  void set_board_size(Size board_size){this->board_size = board_size;}
  Size get_board_size_(){return this->board_size;}
  void set_square_size(float square_size){this->square_size = square_size;}
  float get_square_size_(){return this->square_size;}

 private:
  Mat camera_matrix;
  Mat dist_coeffs;
  Size calibrated_image_size; // Imagesize w x h
